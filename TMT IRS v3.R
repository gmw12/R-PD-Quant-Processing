...
#-----------------------------------------------------------------------------------------------------------
#-----------------------------------------------------------------------------------------------------------
input_data <- "5090_Heart_ProteinPeptide_010319.xlsx"
input_sample_info <- "5090 TMT 010319 Sample Info.xlsx"
...
# functionality to reduce data to common proteins at peptide level before SL normalization
common <- FALSE
if(common){
  ...
  for(i in 1:sets_tmt){
    ...
    assign(str_c("Raw",i,"_peptide"), subset(temp_peptide, temp_peptide$Accession %in% common_protein))
  }
}
