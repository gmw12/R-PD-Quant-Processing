...
file.copy("TMT IRS_v2.R", str_c(output_dir, "TMT IRS_v2.R"))
...
for(i in 1:sets_tmt){
  ...
  assign(str_c("IRS",i,"_peptide"), temp_data)
...
temp_data <- get(str_c("IRS",1,"_peptide"))
target <- colSums(temp_data[(info_count+1):ncol(temp_data)])
for(i in 2:sets_tmt){
  temp_data <- get(str_c("IRS",i,"_peptide"))
...
for(i in 1:sets_tmt){
  ...
  assign(str_c("IRS",i,"_SL_peptide"), temp_sl)
  excel_gw(temp_sl, str_c("IRS",i, "_SL_peptide.xlsx"))
}

#Filter peptides based on CV of SPQC
for(i in 1:sets_tmt){
  temp_data <- get(str_c("IRS",i,"_SL_peptide"))
  ...
  color_list<-irs_set$colorlist
  barplot_gw(temp_data[(info_count+1):ncol(temp_data)], str_c("SL_Peptide_Filtered_",i))
  assign(str_c("IRS",i,"_SL_filtered_peptide"), temp_data)
  excel_gw(temp_data, str_c('SL_filtered',i,'_peptide.xlsx'))
